...
# train for one epoch
def train(net, data_loader, train_optimizer, epoch, writer, args):
    net.train()
    adjust_learning_rate(train_optimizer, epoch, args)

    total_loss, total_num, train_bar = 0.0, 0, tqdm(data_loader)
    for i, (im_1, im_2) in enumerate(train_bar):
        ...
        writer.add_scalar("loss", total_loss / total_num, (epoch-1)*97+i)
...
def main():
    ...
    # data prepare
    train_data = CIFAR100Pair(root='data', train=True, transform=train_transform, download=False)
    train_loader = DataLoader(train_data, batch_size=args.batch_size, shuffle=True, num_workers=16, pin_memory=True,
                              drop_last=True)

    memory_data = CIFAR100(root='data', train=True, transform=test_transform, download=False)
    memory_loader = DataLoader(memory_data, batch_size=args.batch_size, shuffle=False, num_workers=16, pin_memory=True)

    test_data = CIFAR100(root='data', train=False, transform=test_transform, download=False)
    ...
    # training loop
    writer = SummaryWriter("summary_pic")
    for epoch in range(epoch_start, args.epochs + 1):
        train_loss = train(model, train_loader, optimizer, epoch, writer, args)
